...
export default class StarterSelectUiHandler extends MessageUiHandler {
  ...
  setup() {
    ...
    var PandemicNum = this.scene.mods.pokerusPandemic? 5e3 : 3
    this.pokerusCursorObjs = new Array(PandemicNum).fill(null).map(() => {
      ...
      if (button === Button.ACTION) {
        if (!this.speciesStarterDexEntry?.caughtAttr) {
          ...
          const showUseCandies = () => {
            const options = [];
            if (!(passiveAttr & PassiveAttr.UNLOCKED)) {
              const passiveCost = Math.ceil(this.scene.mods.candyCostMultiplier * getPassiveCandyCount(speciesStarters[this.lastSpecies.speciesId]));
              options.push({
            ...
            if (passiveAttr & PassiveAttr.UNLOCKED) {
              const sameSpeciesEggCost = Math.ceil(this.scene.mods.candyCostMultiplier * getSameSpeciesEggCandyCounts(speciesStarters[this.lastSpecies.speciesId]));
              options.push({
                ...
                handler: () => {
                  if (this.scene.gameData.eggs.length < 99 && (Overrides.FREE_CANDY_UPGRADE_OVERRIDE || candyCount >= sameSpeciesEggCost)) {
                    ...
                    this.pokemonCandyCountText.setText(`x${starterData.candyCount}`);
                    const eggOptions: IEggOptions = {scene: this.scene, species: this.lastSpecies.speciesId, sourceType: EggSourceType.SAME_SPECIES_EGG};
                    eggOptions.hatchWaves = this.scene.mods.overrideEggHatchWaves? 1 : eggOptions.hatchWaves;
