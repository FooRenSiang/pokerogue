...
const modifierPool: ModifierPool = {
  ...
  [ModifierTier.ULTRA]: [
    ...
    new WeightedModifierType(modifierTypes.AMULET_COIN, 10),
    ...
    new WeightedModifierType(modifierTypes.CANDY_JAR, 6),
    new WeightedModifierType(modifierTypes.ATTACK_TYPE_BOOSTER, 10),
    new WeightedModifierType(modifierTypes.TM_ULTRA, 8),
    new WeightedModifierType(modifierTypes.RARER_CANDY, 4),
    new WeightedModifierType(modifierTypes.GOLDEN_PUNCH, 10),
    ...
    new WeightedModifierType(modifierTypes.FORM_CHANGE_ITEM, (party: Pokemon[]) => !party[0].scene.mods.formChangeRarity ? 0 : 10),
    ...
    new WeightedModifierType(modifierTypes.DYNAMAX_BAND, (party: Pokemon[]) => !party[0].scene.mods.formChangeRarity ? 0 : Math.min(Math.ceil(party[0].scene.currentBattle.waveIndex / 50), 4) * 8, 32),
  ].map(m => {
    m.setTier(ModifierTier.ULTRA); return m;
  }),
  [ModifierTier.ROGUE]: [
    new WeightedModifierType(modifierTypes.ROGUE_BALL, 24),
    new WeightedModifierType(modifierTypes.RELIC_GOLD, skipInLastClassicWaveOrDefault(3)),
  ...
  [ModifierTier.MASTER]: [
    ...
    new WeightedModifierType(modifierTypes.DNA_SPLICERS, (party: Pokemon[]) => !party[0].scene.gameMode.isSplicedOnly && !party[0].scene.mods.formChangeRarity && party.filter(p => !p.fusionSpecies).length > 1 ? 24 : 0, 24),
...
export function getPartyLuckValue(party: Pokemon[]): integer {
  var luck = Phaser.Math.Clamp(party.map(p => p.isFainted() ? 0 : p.getLuck())
    .reduce((total: integer, value: integer) => total += value, 0), 0, 14);
  luck = party[0].scene.mods.maxLuck == 1 ? 14 : luck;
  return luck || 0;
}
